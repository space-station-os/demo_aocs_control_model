...
/**
 * @brief Computes the L_p vector based on four control surface deflections.
 *
 * This function calculates a 12-element vector L_p using a series of
 * trigonometric and algebraic operations on the input deflections.
 *
 * @param delta_1 Control surface deflection 1 (rad)
 * @param delta_2 Control surface deflection 2 (rad)
 * @param delta_3 Control surface deflection 3 (rad)
 * @param delta_4 Control surface deflection 4 (rad)
 * @param L_p Output 12-element vector (array of double) that is computed from the deflections.
 */
void L_p_func(double delta_1, double delta_2, double delta_3, double delta_4,
              double L_p[12])
{
  // Declaration of intermediate temporary variables used in computation
  double b_t210_tmp;
  double b_t211_tmp;
  double b_t215_tmp;
  double b_t92_tmp;
  ...
  // Compute basic trigonometric functions for the four deflections
  t2 = std::cos(delta_1);
  t3 = std::cos(delta_2);
  t4 = std::cos(delta_3);
  t5 = std::cos(delta_4);
  ...
  // Squares of cosine and sine values for reuse in multiple terms
  t10 = t2 * t2;
  t11 = t3 * t3;
  t12 = t4 * t4;
  t13 = t5 * t5;
  ...
  // Mixed product terms scaled by a large constant
  ...
  // Compute products of squares scaled by constants
  t18_tmp = t10 * t11;
  ...
  // Additional mixed terms involving products of sine and cosine
  t217 = t2 * t6;
  t76_tmp = t217 * t11;
  t78_tmp = t217 * t13;
  t213 = t4 * t8;
  ...
  // Further scaling of intermediate terms for later use
  ...
  // More complex product terms combining multiple trigonometric products
  ...
  // Calculate t210 using a combination of previously computed terms and additional scaling
  ...
  // Compute t211 using additional mixed trigonometric terms
  ...
  // Compute t212 using additional cross product terms
  t210_tmp = t217 * t12;
  t212_tmp = t213 * t10;
  ...
  // Compute t213 using a large combination of previously computed terms and scaling factors
  ...
  // Compute final L_p values based on the scaled intermediate terms
